...
void precompute_factorials(int maxN = maxn, ll m = MOD)
{
...
ll getFvalue(int l, int r, ll sum, int pos)
{
    if (pos == r + 1)
    {
        return factorial[sum % maxn];
    }

    ll ans = 0ll;

    ans += getFvalue(l, r, sum, pos + 1) % MOD;
    ans += getFvalue(l, r, sum + v[pos], pos + 1) % MOD;
...
int main()
{
    ...
    for (ll i = 3; i < maxn; i++)
    {
        factorial[i] = (factorial[i - 1] + factorial[i - 2]) % MOD;
    }
