...
{
    ll sum;
    int lazyInc; // Lazy increment propagation

public:
    Node() : sum(0), lazyInc(0) {};
};
...
void propagate(int idx, int low, int high)
{
    if (segTree[idx].lazyInc != 0)
    {
        ...
        if (low != high)
        {
            segTree[2 * idx + 1].lazyInc = (segTree[2 * idx + 1].lazyInc + segTree[idx].lazyInc) % 2;
            segTree[2 * idx + 2].lazyInc = (segTree[2 * idx + 2].lazyInc + segTree[idx].lazyInc) % 2;
        }
...
void update(int idx, int low, int high, int l, int r)
{
    ...
    { // Completely inside range
        segTree[idx].lazyInc = !segTree[idx].lazyInc;
        propagate(idx, low, high);
        return;
    }
